...
public:
    ...
    // Function : sort linked list values
    void sort() {
    ...
    void displayWithAddress(){
        Node<T>* temp = head;
        while (temp) {
            // reader-friendly display of node data along with its memory address( hex value to int)
            cout << "[" << temp->data << " | " << abs((int)(uintptr_t)temp->next % 10000) << "] -> ";
            temp = temp->next;
        }
        cout << "NULL\n";
    }
